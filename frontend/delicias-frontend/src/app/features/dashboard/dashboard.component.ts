...
export class DashboardComponent {
  ...
  monthLabel = computed(() => `${this.months[this.curMonth()]} ${this.curYear()}`);

  encomendas = computed(() => this.encService.getByMes(this.curYear(), this.curMonth()));
  vendasUnit = computed(() => this.vendaService.getByMes(this.curYear(), this.curMonth()));
  ...
  receitaSemanas = computed(() => {
    ...
    return semanas.map(s => {
      const enc = this.encomendas()
        .filter(e => {
          const d = new Date(e.data_entrega).getDate();
          return d >= s.dias[0] && d <= s.dias[1] && e.status !== 'cancelado';
        })
        .reduce((sum,e) => sum + e.valor, 0);
      const unit = this.vendasUnit()
        .filter(v => {
          const d = new Date(v.data_venda).getDate();
          return d >= s.dias[0] && d <= s.dias[1];
        })
  ...
  maxSemana = computed(() => Math.max(...this.receitaSemanas().map(s => s.enc + s.unit), 1));

  topProdutos = computed(() => {
    const contagem: Record<string, number> = {};
    this.encomendas().forEach(e => {
      contagem[e.tipo_encomenda] = (contagem[e.tipo_encomenda] || 0) + 1;
    });
